...
export function pdfTo(options: { input: InputType; output: Writable; to: ConvertTo }): void;
export function pdfTo(options: { input: InputType; to: ConvertTo }): Promise<Buffer>;
export function pdfTo({ input, output, to }: { input: InputType; output?: Writable; to: ConvertTo }) {
  const command = (() => {
    switch (to) {
      case 'text':
